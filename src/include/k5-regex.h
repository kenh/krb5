 ...
 *
 * We only define REG_NOMATCH, since no Kerberos code looks for anything other
 * than success and REG_NOMATCH.  Any exceptions from these functions are
 * caught and return the appropriate error code which should be decoded
 * by k5_regerror();
 */
...
int k5_regexec(const regex_t *preg, const char *string, size_t,
